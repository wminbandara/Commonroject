...
{
    public partial class FormSearchVehicleMilage : Form
    {
        public FormSearchVehicleMilage()
        {
            InitializeComponent();
        }
        ...
        private void updatecreditpaycancel()
        {
            try
            {
                ...
                objBAL.VehicleMilageHDId = Convert.ToInt32(txtReprint.Text);
                objBAL.ModifiedBy = Convert.ToInt32(lblUserId.Text);
                ClassMasterDAL objDAL = new ClassMasterDAL();
                int count = objDAL.updateVehicleLogCancel(objBAL);
                if (count != 0)
                {
                    MessageBox.Show("Vehicle Log cancelled Susccessfully.", "Save Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void fillGrid()
        {
            ...
            if (objDAL.retreiveAllVehicleLog(objBAL).Tables[0].Rows.Count > 0)
            {
        ...
        private void ReprintInvoice()
        {
            try
            {
                ...
                CrystalReportVehicleLogPrint rpt = new CrystalReportVehicleLogPrint();
                ClassPOBAL objPOBAL = new ClassPOBAL();
                objPOBAL.VehicleMilageHDId = Convert.ToInt32(txtReprint.Text);
                ClassPODAL objPODAL = new ClassPODAL();
                objPOBAL.DtDataSet = objPODAL.retreiveVehicleLogData(objPOBAL);
        ...
        {
            txtReprint.Text = (this.gridView1.GetFocusedRowCellValue("VehicleMilageHDId").ToString());
            ReprintInvoice();
        }

        private void FormSearchVehicleMilage_Load(object sender, EventArgs e)
        {
            Cursor.Current = Cursors.WaitCursor;
            fillGrid();
            Cursor.Current = Cursors.Default;
        ...
        {
            if (this.gridView1.GetFocusedRowCellValue("VehicleMilageHDId") == null)
                return;
            fillInv();
        }
        ...
        {
            DialogResult result = MessageBox.Show("Do you want to Cancel this Entry ", "Cancellation Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
