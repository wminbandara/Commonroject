...
{
    public partial class FormGIN : Form
    {
        ...
        private void insertPIHD()
        {
            try
            {
                ...
                objDAL = new ClassPODAL();
                string count = objDAL.InsertNewGINHD(objBAL);
        ...
        private void insertPIDT()
        {
            try
            {
                ...
                {
                    resetDetail();
                    MessageBox.Show("BOM Details Saved Successfully.", "Save Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void FormGIN_Load(object sender, EventArgs e)
        {
            formLoad();
        }
        ...
        private void FormGIN_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.F2)
            {
                textBoxItemCode.Select();
                FormItemSearch frm11 = new FormItemSearch();
                frm11.frm11 = this;
                frm11.form = 11;
                frm11.wh = Convert.ToInt32(comboBoxBranch.SelectedValue.ToString());
                frm11.lblUser.Text = lblUser.Text.Trim();
                frm11.lblUserId.Text = lblUserId.Text;
                frm11.ShowDialog(this);
            }
        }

        private void textBoxFGItemode_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                try
                    ...
                    if (objPOBAL.DtDataSet.Tables[1].Rows.Count > 0)
                    {
                        ...
                        foreach (DataRow dRow in objPOBAL.DtDataSet.Tables[1].Rows)
                        {
                            ...
                            foreach (object value in dRow.ItemArray)
                                values.Add(value);
                            newval.Add(values);
                            textBoxFGItemName.Text = (values[1].ToString().Trim());
                            textBoxFGSellingPrice.Text = (values[5].ToString().Trim());
                            textBoxFGItemId.Text = (values[7].ToString().Trim());
                            textBoxFGQty.Text = "1";
                        }
                        textBoxItemCode.Select();
                    }
