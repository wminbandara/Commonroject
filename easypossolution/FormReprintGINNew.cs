...
{
    public partial class FormReprintGINNew : Form
    {
        ...
        private void fillTransferGrid()
        {
            ...
            if (objDAL.retreiveAllGINByID(objBAL).Tables[0].Rows.Count > 0)
            {
        ...
        private void updatecancel()
        {
            try
            {
                ...
                int count = objDAL.updateGINNewCancel(objBAL);
                if (count != 0)
                {
                    MessageBox.Show("GIN cancelled Susccessfully.", "Save Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void fillGrid()
        {
            ...
            if (objDAL.retreiveAllGINNew(objBAL).Tables[0].Rows.Count > 0)
            {
        ...
        {
            txtReprint.Text = (this.gridView1.GetFocusedRowCellValue("GINId").ToString());
            ReprintInvoice();
        }

        #endregion

        private void FormReprintGINNew_Load(object sender, EventArgs e)
        {
            Cursor.Current = Cursors.WaitCursor;
            fillGrid();
            Cursor.Current = Cursors.Default;
        ...
        {
            if (this.gridView1.GetFocusedRowCellValue("GINId") == null)
                return;
            fillInv();
        }
        ...
        {
            DialogResult result = MessageBox.Show("Do you want to Cancel this GIN ", "Cancellation Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
