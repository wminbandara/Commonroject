...
{
    public partial class FormGINNew : Form
    {
        ...
        private void SaveHDNew()
        {
            try
            {
                ...
                objInvBAL.TotalCost = Convert.ToDecimal(textBoxTotalCost.Text);

                ClassInvoiveDAL objInvDAL = new ClassInvoiveDAL();
                string count = objInvDAL.InsertGINhdNew(objInvBAL);
        ...
        private void insertPIDT()
        {
            try
            {
                for (int i = 0; i < dgView.Rows.Count; i++)
                {
                    ...
                    objBAL.PurchasePrice = Convert.ToDecimal(dgView.Rows[i].Cells["SellingPrice"].Value);
                    objBAL.SellingPrice = Convert.ToDecimal(dgView.Rows[i].Cells["PurchaseQty"].Value);
                    ...
                    int count = objDAL.InsertGINDt(objBAL);
                    if (count != 0)
                    {
                        insertDTStatus = true;
                    }
                ...
                {
                    resetDetail();
                    MessageBox.Show("GIN Saved Successfully.", "Save Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    print();
                    Reset();
                    fillGrid();
                }
        ...
        private void print()
        {
            try
            {
                ...
                REPORT.Show();
                CrystalReportGINNew rpt = new CrystalReportGINNew();
                objBAL = new ClassPOBAL();
                objBAL.TransferHDId = Convert.ToInt32(TransferHDId);
                objDAL = new ClassPODAL();
                objBAL.DtDataSet = objDAL.retreiveGINPrintNewData(objBAL);
        ...
        void CalculateTotal()
        {
            try
            {
                ...
                textBoxTotalCost.Text = GrossTot.ToString("0.00");
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
        ...
        private void LoadExpCat()
        {
            try
            {
                ClassCommonBAL objBAL = new ClassCommonBAL();
                ClassMasterDAL objDAL = new ClassMasterDAL();
                comboBoxWarehouseTo.DataSource = objDAL.retreiveExpenseCat(objBAL).Tables[0];
                comboBoxWarehouseTo.DisplayMember = "CatDescription";
                comboBoxWarehouseTo.ValueMember = "PayCatId";
                comboBoxWarehouseTo.SelectedIndex = -1;
            }
            catch
            {
            }
        ...
        private void FormGINNew_Load(object sender, EventArgs e)
        {
            try
            {
                loadStatus = true;
        ...
        private void FormGINNew_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.F2)
            {
                textBoxItemCode.Select();
                FormItemSearch frm17 = new FormItemSearch();
                frm17.frm17 = this;
                frm17.form = 17;
                frm17.wh = Convert.ToInt32(comboBoxWarehouse.SelectedValue.ToString());
                frm17.lblUser.Text = lblUser.Text.Trim();
                frm17.lblUserId.Text = lblUserId.Text.Trim();
